...
# # Pivoting the table to create a user-item matrix
...
# user_preference_vectors.head()

# Filter data to include only the top 1000 most frequently rated items
top_items = df['Item'].value_counts().nlargest(1000).index
filtered_df = df[df['Item'].isin(top_items)]

# Filter data to include only the top 1000 users who have rated the most items
